import { VIOLATION_TYPES, getPolicy, type ViolationType } from '../../domain/policies'

export function buildTriagePrompt(review: { text: string; rating: number; authorName: string }): string {
  const catalog = VIOLATION_TYPES.map((t) => `- ${t}: ${getPolicy(t as ViolationType).description}`).join('\n')
  return [
    ...
    'Violation catalog:',
    catalog,
    'Return strict JSON: {"violationType": <one of the catalog keys or null>, "caseStrength": "HIGH"|"MEDIUM"|"NONE", "confidence": 0..1}',
